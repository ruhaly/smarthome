...
public class URLEncodedUtils {
	...
	/**
	 * Returns a list of {@link org.apache.http.NameValuePair NameValuePairs} as
	 * built from the URI's query portion. For example, a URI of
	 ...
	 * <code>parameters</code>. For example,a scanner containing the string
	 * <code>a=1&b=2&c=3</code> would add the
	 * {@link org.apache.http.NameValuePair NameValuePairs} a=1, b=2, and c=3 to
	 ...
	 * @return encoded string
	 */
	public static String decodeFormFields(final String content,
			final String charset) {
		if (content == null) {
			return null;
		}
		return urldecode(content, charset != null ? Charset.forName(charset)
				: Charset.forName(HTTP.UTF_8), true);
	}
	...
	public static String decodeFormFields(final String content,
		...
		return urldecode(content,
				charset != null ? charset : Charset.forName(HTTP.UTF_8), true);
	}

	/**
	 * Encode/escape www-url-form-encoded content.
	 * <p/>
	 * Uses the {@link #URLENCODER} set of characters, rather than the set; this
	 * is for compatibilty with previous releases, URLEncoder.encode() and most
	 * browsers.
	 * 
	 * @param content
	...
	private static String encodeFormFields(final String content,
		...
		return urlencode(content,
				charset != null ? charset : Charset.forName(HTTP.UTF_8),
				URLENCODER, true);
	}
