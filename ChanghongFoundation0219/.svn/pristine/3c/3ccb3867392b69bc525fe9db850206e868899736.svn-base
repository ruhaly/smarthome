 ...
 */
public class MsgHuiminLogic extends SuperLogic
{
    
    private static MsgHuiminLogic ins;
    
    HttpHandler<String> httpHanlder;
    
    public List<MessageInfo> huiminMsgList = new ArrayList<MessageInfo>();
    
    public static final int PAGENUM = 10;
    
    public int currentPage = 1;
    
    public static synchronized MsgHuiminLogic getInstance()
    {
        if (null == ins)
        {
            ins = new MsgHuiminLogic();
        }
        return ins;
    }
    
    public void requestHuiminMsgList(BaseAccountInfo baseAccountInfo,
            int[] type, HttpUtils httpUtils)
    {
        ...
        httpHanlder = HttpSenderUtils.sendMsgImpl(HttpAction.ACTION_GET_MSGLIST,
                ...
                httpUtils,
                RequestId.HUIMINMSGLIST_REQUESTID,
                this,
                false,
                HttpUrl.URL_CBS);
            ...
            InputStream is)
    {
        ...
        switch (requestId)
        {
            case RequestId.HUIMINMSGLIST_REQUESTID:
                handlerHuiminMsgList(serviceRes);
                return;
        }
    }
    
    private void handlerHuiminMsgList(ServiceResponse response)
    {
        // TODO Auto-generated method stub
        if (response.getBody().getResult().equals("0"))
        {
            huiminMsgList = JsonParse.parseMsgListRes(response.getBody()
                    .getParamsString());
            if (null != huiminMsgList)
            {
                handler.sendEmptyMessage(MsgWhat.MSGWHAT_HUIMINMSG_SUCCESS);
            }
            else
            {
