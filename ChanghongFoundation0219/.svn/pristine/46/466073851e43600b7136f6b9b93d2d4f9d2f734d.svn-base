 ...
 */
public class MsgWuyeLogic extends SuperLogic
{
    
    private static MsgWuyeLogic ins;
    
    HttpHandler<String> httpHanlder;
    
    public List<MessageInfo> tenementMsgList = new ArrayList<MessageInfo>();
    
    public static final int PAGENUM = 10;
    
    public int currentPage = 1;
    
    public static synchronized MsgWuyeLogic getInstance()
    {
        if (null == ins)
        {
            ins = new MsgWuyeLogic();
        }
        return ins;
    }
    
    public void requestTenementMsgList(BaseAccountInfo baseAccountInfo,
            int[] type, HttpUtils httpUtils)
    {
        ...
        httpHanlder = HttpSenderUtils.sendMsgImpl(HttpAction.ACTION_GET_MSGLIST,
                ...
                httpUtils,
                RequestId.TENEMENTMSGLIST_REQUESTID,
                this,
                false,
                HttpUrl.URL_CBS);
            ...
            InputStream is)
    {
        ...
        switch (requestId)
        {
            ...
            case RequestId.TENEMENTMSGLIST_REQUESTID:
                handlerTenementMsgList(serviceRes);
                return;
                
        }
    }
    
    private void handlerTenementMsgList(ServiceResponse response)
    {
        // TODO Auto-generated method stub
        if (response.getBody().getResult().equals("0"))
        {
            tenementMsgList = JsonParse.parseMsgListRes(response.getBody()
                    .getParamsString());
            if (null != tenementMsgList)
            {
                handler.sendEmptyMessage(MsgWhat.MSGWHAT_TENEMENTMSG_SUCCESS);
            }
            else
            {
