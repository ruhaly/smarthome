...
import com.changhong.smarthome.phone.cinema.activity.PlayerDetailActivity;
...
public class MediaController extends FrameLayout
{
    ...
    private ImageButton mTransfromButton;//全屏和非全屏之间的转换按钮
    
    private ImageButton mediacontroller_play_previous;//�上一个
    private ImageButton mediacontroller_play_next;//下一个
    private ImageButton mediacontroller_slipline;//分割线
    private ImageButton mediacontroller_pullsrceen;//上传到tv端播放按钮
    ...
    private View.OnClickListener mTransfromListener = new View.OnClickListener()
    {
        public void onClick(View v)
        {
            if (((PlayerDetailActivity) mContext).getRequestedOrientation() == ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE)
            {
                ((PlayerDetailActivity) mContext).setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
                
                mediacontroller_play_previous.setVisibility(View.GONE);
                mediacontroller_play_next.setVisibility(View.GONE);
               /* mediacontroller_slipline.setVisibility(View.GONE);
                mediacontroller_pullsrceen.setVisibility(View.GONE);*/
                mFileName.setVisibility(View.GONE);
              
            }else{
                ((PlayerDetailActivity) mContext).setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE);
                mediacontroller_play_previous.setVisibility(View.VISIBLE);
                mediacontroller_play_next.setVisibility(View.VISIBLE);
               /* mediacontroller_slipline.setVisibility(View.VISIBLE);
                mediacontroller_pullsrceen.setVisibility(View.VISIBLE);*/
                mFileName.setVisibility(View.VISIBLE);
                
            }
            
            //((Activity)mContext).finish();
             //((Activity)mContext).ifornotfullscreen();
        }
    };
    ...
    public void setWindowLayoutType()
    {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH)
        {
            try
            {
                ...
                Method setWindowLayoutType = PopupWindow.class.getMethod("setWindowLayoutType",
                        new Class[] { int.class });
                setWindowLayoutType.invoke(mWindow,
    ...
    {
        return ((LayoutInflater) mContext.getSystemService(Context.LAYOUT_INFLATER_SERVICE)).inflate(R.layout.mediacontroller,
                this);
    }
    ...
    private void initControllerView(View v)
    {
        ...
        //�Զ��尴ť
        mediacontroller_play_previous = (ImageButton) v.findViewById(R.id.mediacontroller_play_previous);
        mediacontroller_play_next = (ImageButton) v.findViewById(R.id.mediacontroller_play_next);
        mediacontroller_slipline = (ImageButton) v.findViewById(R.id.mediacontroller_slipline);
        mediacontroller_pullsrceen = (ImageButton) v.findViewById(R.id.mediacontroller_pullsrceen);
        ...
        mTransfromButton = (ImageButton) v.findViewById(R.id.mediacontroller_play_transform);
        ...
        if (mTransfromButton != null)
        {
            mTransfromButton.setOnClickListener(mTransfromListener);
        }
    ...
    public void show(int timeout)
    {
        if (!mShowing && mAnchor != null && mAnchor.getWindowToken() != null)
        {
            ...
            else
            {
                ...
                Rect anchorRect = new Rect(location[0], 0,
                        location[0] + mAnchor.getWidth(), mAnchor.getHeight()-location[1]/3);
                
                mWindow.setAnimationStyle(mAnimStyle);
                setWindowLayoutType();
                mWindow.showAtLocation(mAnchor,
                        Gravity.NO_GRAVITY,
                        anchorRect.left,
                        anchorRect.bottom);
            }
    ...
    private void updatePausePlay()
    {
        ...
        if (mPlayer.isPlaying())
        {
            mPauseButton.setBackgroundResource(R.drawable.mediacontroller_pause_button);
        }
            //mPauseButton.setImageResource(R.drawable.mediacontroller_pause_button);
        else{
            mPauseButton.setBackgroundResource(R.drawable.mediacontroller_play_button);
        }
